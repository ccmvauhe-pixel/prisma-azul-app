/**
 * Significados de las 40 cartas de la baraja española, según la lectura de Gilda.
 * Portado de `baraja-data.js` (fuente: PDFs). Contenido literal.
 */
...
export type CartaExtra = { t: string; x: string };

export type CartaSignificado = {
  general: string;
  amor: string;
  trabajo: string;
  dinero: string;
...
export const PALOS: PaloDef[] = [
  {
    "key": "oros",
    "nombre": "Oros",
    "elemento": "Tierra · Prosperidad",
    "color": "#ecc874",
    "idxColor": "#f0d488"
  },
  {
...
/** La baraja española de 40 cartas no tiene 8 ni 9 */
export const SIN_CONTENIDO: number[] = [
  8,
  9
];

/** Clave: `${palo}-${num}`, p. ej. `oros-1` */
export const CARTAS: Record<string, CartaSignificado> = {
  "oros-1": {
  ...
  "espadas-2": {
    ...
    "extras": [
      {
        "t": "Energía negativa",
        "x": "Presencia de energías negativas o personas actuando en tu contra a través de sus pensamientos, intenciones, deseos o acciones. Aunque no es tan intensa, sigue siendo fuerte. Mantente alerta, realiza una limpieza energética y no permitas que las malas intenciones afecten tu equilibrio."
      }
    ]
  },
  "espadas-3": {
  ...
  "espadas-5": {
    ...
    "extras": [
      {
        "t": "Salud",
        "x": "Presta atención a tu bienestar y no dejes pasar malestares o síntomas. Acude a revisión médica, hazte los estudios necesarios o da seguimiento a un tratamiento antes de que se complique. Puede representar trámites de salud: consultas, análisis clínicos o procedimientos. Atiende el problema a tiempo."
      }
    ]
  },
  "espadas-6": {
    ...
    "extras": [
      {
        "t": "Salud",
        "x": "Preocupación por la salud, propia o de alguien cercano. Presta más atención al bienestar físico y emocional, sin dejar que el miedo o la ansiedad se apoderen de la situación."
      }
    ]
  },
  "espadas-7": {

    ...
    <sc-if value="{{ isCamino }}" hint-placeholder-val="{{ true }}">
      <div data-screen-label="Mi Camino" style="position:relative;min-height:100%;background:radial-gradient(135% 80% at 50% -10%, #241a4f 0%, #150d34 46%, #0a0720 100%);padding:26px 22px 40px">
        ...
        <div style="position:relative">
          ...
          <div onClick="{{ resetLecturas }}" style="display:block;margin:18px auto 0;width:fit-content;text-align:center;font-size:12px;color:rgba(205,189,242,.5);cursor:pointer;text-decoration:underline;padding:10px">Restablecer lecturas (demo)</div>
        </div>
      </div>
    </sc-if>
...
class Component extends DCLogic {
  ...
  renderVals() {
    ...
    return {
      ...
      resetLecturas: () => {
        try {
          const fijas = ['pa_vibra','pa_afirmacion_lock','pa_afirmacion_last','pa_afirmacion_hist','pa_codigo_activo','pa_oraculo_lock','pa_oraculo_last','pa_oraculo_hist','pa_cruz_last','pa_cruz_mes','pa_cruz_next'];
          fijas.forEach(k => localStorage.removeItem(k));
          Object.keys(localStorage).filter(k => k.indexOf('pa_codigo_reps_')===0).forEach(k => localStorage.removeItem(k));
        } catch(e){}
        this.setState({now:Date.now(), toast:'✨ Lecturas restablecidas'});
